...
export function validateEnvironment(): ValidationResult {
  ...
  for (const varName of optionalVars) {
    ...
    if (!value || value.trim() === "") {
      warnings.push(
        `Optional environment variable missing: ${varName} - some functionality may be limited`,
      );
    }
  }
  ...
  if (
    supabaseUrl && !supabaseUrl.match(/^https:\/\/[a-z0-9]+\.supabase\.co$/)
  ) {
    errors.push(
      "SUPABASE_URL must be in format: https://[project-id].supabase.co",
    );
  }
  ...
  if (openrouterKey && !openrouterKey.startsWith("sk-or-")) {
    warnings.push(
      'OPENROUTER_API_KEY should start with "sk-or-" - please verify this is correct',
    );
  }
  ...
  if (Deno.env.get("SUPABASE_URL") === Deno.env.get("SUPABASE_ANON_KEY")) {
    errors.push(
      "SUPABASE_URL and SUPABASE_ANON_KEY appear to be the same - please check your configuration",
    );
  }

...
Deno.test("firecrawl-extract - Quattro Integration Test", async (t) => {
  await t.step("should extract Quattro product data successfully", async () => {
    const testURL = "https://magnumsupps.com/en-us/products/quattro?variant=46056179892527";
    ...
    assertEquals(validSources.includes(result._meta.source), true, 
    ...
    assertStringIncludes(result.title.toLowerCase(), "quattro", 
    ...
    assertEquals(result.ingredients_raw.length > 0, true, 
      "ingredients_raw should not be empty");
    assertStringIncludes(result.ingredients_raw.toLowerCase(), "isolated protein", 
    ...
    console.log(`   Ingredients length: ${result.ingredients_raw?.length || 0} characters`);
  ...
  await t.step("should handle extraction failures gracefully", async () => {
    const invalidURL = "https://magnumsupps.com/en-us/products/nonexistent-product";
    ...
    try {
      ...
      assertEquals(result.title === null || result.title === "", true, 
        "Should return null/empty title for invalid URL");
    } catch (error) {
      // Error handling is acceptable for invalid URLs
      console.log(`Expected error for invalid URL: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  
  await t.step("should provide debug information on extraction failure", async () => {
    const testURL = "https://magnumsupps.com/en-us/products/quattro?variant=46056179892527";
    ...
    try {
      ...
      if (!result.title || !result.ingredients_raw) {
        ...
        if (result.html) {
          console.error(`   HTML preview: ${result.html.substring(0, 500)}...`);
        }
